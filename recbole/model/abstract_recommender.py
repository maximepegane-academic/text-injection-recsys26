...
class AbstractRecommender(nn.Module):
    ...
    def other_parameter(self):
        if hasattr(self, "id2token") and hasattr(self, "token2id"):
            return {"id2token": self.id2token, "token2id": self.token2id}
        return dict()

    def load_other_parameter(self, para):
        if para is not None:
            self.id2token = para.get("id2token", {})
            self.token2id = para.get("token2id", {})
...
class SequentialRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.internal_to_original_user = self._build_iso_map(
            dataset, "user_feat", self.USER_ID, "original_shopper_id"
        )
        self.internal_to_original_item = self._build_iso_map(
            dataset, "item_feat", self.ITEM_ID, "original_product_id"
        )

    def other_parameter(self):
        return {
            "id2token": getattr(self, "id2token", {}),
            "token2id": getattr(self, "token2id", {}),
            "internal_to_original_user": getattr(self, "internal_to_original_user", {}),
            "internal_to_original_item": getattr(self, "internal_to_original_item", {}),
        }
    ...
    def load_other_parameter(self, para):
        if para is not None:
            self.id2token = para.get("id2token", {})
            self.token2id = para.get("token2id", {})
            self.internal_to_original_user = para.get("internal_to_original_user", {})
            self.internal_to_original_item = para.get("internal_to_original_item", {})

    def _build_iso_map(self, dataset, feat_name, internal_field, orig_field):
        feat = getattr(dataset, feat_name, None)
        if (
            feat is not None
            and orig_field in feat.columns
            and orig_field in getattr(self, "id2token", {})
        ):
            uids = (
                feat[internal_field].to_list()
                if isinstance(feat, pl.LazyFrame) or isinstance(feat, pl.DataFrame)
                else feat[internal_field].flatten().cpu().tolist()
            )
            orig_uids = (
                feat[orig_field].to_list()
                if isinstance(feat, pl.LazyFrame) or isinstance(feat, pl.DataFrame)
                else feat[orig_field].flatten().cpu().tolist()
            )
            return {
                u: self.id2token[orig_field][orig_u]
                for u, orig_u in zip(uids, orig_uids)
            }
        return {}
    ...
    def gather_indexes(self, output, gather_index):
        ...
        gather_index = (
            gather_index.view(-1, 1, 1).expand(-1, -1, output.shape[-1]).clamp(0)
        )
