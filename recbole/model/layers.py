...
class UserBias(nn.Module):
    ...
    def forward(self, item_seq, items=None):
        batch_size = item_seq.shape[0]
        if items is None:
            logits = torch.zeros([batch_size, self.n_items])
            valid_items = (item_seq > 0).float()
            n_items = valid_items.sum(dim=1, keepdim=True)
            logits.scatter_add_(index=item_seq, src=valid_items, dim=1)
            log_prob = torch.log(logits / (n_items + 1e-9))
            log_prob = (log_prob - log_prob.min(dim=1, keepdim=True).values) / (
                    log_prob.max(dim=1, keepdim=True).values - log_prob.min(dim=1, keepdim=True).values)

            bias = (1 + self.alpha) * log_prob + self.beta
            bias[:, 0] = 0.0
            return bias
        sorted_seq, _ = torch.sort(item_seq, dim=1)
        left_idx = torch.searchsorted(sorted_seq, items, side='left')
        right_idx = torch.searchsorted(sorted_seq, items, side='right')
        counts = (right_idx - left_idx).float()
        counts = torch.where(items == 0, torch.tensor(0.0, device=counts.device), counts)

        log_prob = counts / (counts.sum(dim=1, keepdim=True) + 1e-9)
        log_prob = (log_prob - log_prob.min(dim=1, keepdim=True).values) / (
                log_prob.max(dim=1, keepdim=True).values - log_prob.min(dim=1, keepdim=True).values + 1e-9)
        bias = (1 + self.alpha) * log_prob + self.beta
        bias = torch.where(items == 0, torch.tensor(0.0, device=bias.device), bias)

        return bias

class LogBias(nn.Module):
    """
    Calculates bias based on the normalized log probabilities of item popularity.
    """

    def __init__(self, dataset):
        super().__init__()
        popularity = dataset.item_popularity.float()

        total_count = popularity.sum()
        item_probs = popularity / (total_count + 1e-9)

        log_probs = torch.log(item_probs + 1e-9)
        log_probs = (log_probs - log_probs.min()) / (log_probs.max() - log_probs.min())
    ...
    def forward(self, items=None):
        if items is None:
            bias = (1 + self.alpha) * self.log_probs + self.beta
            bias[0] = 0.0  # Zero out padding token
            return bias

        bias = (1 + self.alpha) * self.log_probs[items] + self.beta
        bias = torch.where(items == 0, torch.tensor(0.0, device=bias.device), bias)
        return bias
...
class EmbeddingBias(nn.Module):
    """
    A simple 1-dimensional embedding table for each item, initialized near zero.
    """

    def __init__(self, n_items):
        super().__init__()
        self.bias_emb = nn.Embedding(n_items, 1, padding_idx=0)
        # Initialize near 0 so it doesn't immediately disrupt the model
        nn.init.normal_(self.bias_emb.weight, mean=0.0, std=0.01)

    def forward(self, items=None):
        if items is None:
            # Squeeze to ensure the output is a 1D tensor [n_items]
            return self.bias_emb.weight.squeeze(-1)

        # Squeeze the last dimension to match the shape of `items`
        return self.bias_emb(items).squeeze(-1)


class MLPBias(nn.Module):
    """
    Calculates item bias by passing an item ID embedding through a two-layer MLP.
    """

    def __init__(self, n_items, hidden_size):
        super().__init__()
        # Initial embedding to represent the item before passing to the MLP
    ...
    def forward(self, items=None):
        if items is None:
            # Calculate bias for all items in the catalog
            all_biases = self.mlp(self.item_emb.weight).squeeze(-1)
            all_biases[0] = 0.0  # Zero out padding token
            return all_biases

        bias = self.mlp(self.item_emb(items)).squeeze(-1)
        bias = torch.where(items == 0, torch.tensor(0.0, device=bias.device), bias)
        return bias
...
class ItemBias(nn.Module):
    """
    Dispatcher module that loads the specified bias module based on the configuration.
    """
    ...
    def __init__(self, dataset, *args, **kwargs):
        ...
        self.bias_type = self.config.get("bias_type", "log").lower()
        n_items = dataset.item_num  # RecBole standard property for catalog size
        if self.bias_type == "log":
            self.bias_module = LogBias(dataset)
        elif self.bias_type == "embedding":
            self.bias_module = EmbeddingBias(n_items)
        elif self.bias_type == "mlp":
            # Grabs hidden_size from config, defaults to 64 if not explicitly set
            hidden_size = self.config.get("hidden_size", 64)
            self.bias_module = MLPBias(n_items, hidden_size)
        else:
            raise ValueError(f"Unknown bias_type: '{self.bias_type}'. Must be 'log', 'embedding', or 'mlp'.")

    def forward(self, items=None):
        return self.bias_module(items)
