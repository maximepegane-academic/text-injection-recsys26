...
class AbstractSampler(object):
    ...
    def sample_by_key_ids(self, key_ids, num):
        ...
        if (key_ids == key_ids[0]).all():
            ...
            if self.dataset.config["optimize"]:
                from itertools import chain

                seen = np.fromiter(
                    chain.from_iterable(self.used_ids),
                    dtype=np.int64,
                    count=len(self.used_ids),
                )
                candidate_pool = torch.ones(self.item_num, dtype=torch.int64)
                if self.distribution == "popularity":
                    candidate_pool = self.dataset.item_popularity
                candidate_pool[seen] = 0
                neg_samples = torch.multinomial(
                    candidate_pool.float()
                    .unsqueeze(1)
                    .repeat(1, self.dataset.max_item_list_len)
                    .T,
                    total_num,
                    replacement=False,
                )
                return neg_samples.T
            else:
    ...
    def sample_by_interaction(self, interaction, num):
        current_items = interaction[self.dataset.iid_field].long()
        history_items = interaction[self.dataset.iid_field + self.dataset.list_suffix].long()
        batch_size = interaction.length
        list_len = self.dataset.max_item_list_len
        candidate_pool = torch.ones(self.item_num, dtype=torch.float)
        if self.distribution == "popularity":
            candidate_pool = self.dataset.item_popularity.float()

        batched_pool = candidate_pool.unsqueeze(0).repeat(batch_size, 1)

        batched_pool.scatter_(dim=1, index=history_items, value=0.0)

        if current_items.dim() == 1:
            current_items = current_items.unsqueeze(1)
        batched_pool.scatter_(dim=1, index=current_items, value=0.0)
        batched_pool = batched_pool.repeat(num, 1)
        neg_samples = torch.multinomial(
            batched_pool,
            list_len,
            replacement=True
        ...
        neg_samples = neg_samples.view(batch_size, list_len, num)
...
class RepeatableSampler(AbstractSampler):
    ...
    def _get_candidates_list(self):
        candidates = self.dataset.inter_feat[self.iid_field].numpy()
        candidates = list(candidates[candidates != 0].flatten())
        return candidates
    ...
    def sample_by_user_ids(self, user_ids, item_ids, num):
        ...
        """
        try:
            self.used_ids = np.array([{i} for i in item_ids[item_ids != 0].flatten()])
