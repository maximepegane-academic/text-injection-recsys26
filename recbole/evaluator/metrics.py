...
class Recall(TopkMetric):
    ...
    def metric_info(self, pos_index, pos_len):
        unsqueezed = np.expand_dims(pos_len, 1)
        result = np.divide(
            np.cumsum(pos_index, axis=1), unsqueezed, where=unsqueezed != 0
        )
        return result


class ContributionRepeatRecall(Recall):
    """
    This metric is similar to the recall metric, except that it computes only repeat item out of all items,
    with the ContributionExploreRecall metric you have the following relation :
    ContributionExploreRecall + ContributionRepeatRecall = Recall
    """
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result("contribution_repeat_recall", result)
        return metric_dict


class RepeatRecall(ContributionRepeatRecall):
    """
    This metric calculates the recall only taking into account the number of positive prediction on repeat items
    """

    def metric_info(self, pos_index, pos_len):
        result = super().metric_info(pos_index, pos_len)
        result = result[pos_len > 0]
        return result
...
class ContributionExploreRecall(Recall):
    """
    This metric is similar to the recall metric, except that it computes only explore item out of all items,
    with the ContributionRepeatRecall metric you have the following relation :
    ContributionExploreRecall + ContributionRepeatRecall = Recall
    """
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result("contribution_explore_recall", result)
        return metric_dict


class ExploreRecall(ContributionExploreRecall):
    """
    This metric calculates the recall only taking into account the number of positive prediction on repeat items
    """
...
class ContributionRepeatNDCG(NDCG):
    metric_need = ["rec.topk_repeat"]
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result("contribution_repeat_ndcg", result)
        return metric_dict


class RepeatNDCG(ContributionRepeatNDCG):
    def metric_info(self, pos_index, pos_len):
        result = super().metric_info(pos_index, pos_len)
        result = result[pos_len > 0]
        return result
...
class ContributionExploreNDCG(NDCG):
    r"""NDCG_ (also known as normalized discounted cumulative gain) is a measure of ranking quality,
    where positions are discounted logarithmically. It accounts for the position of the hit by assigning
    higher scores to hits at top ranks.
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result("contribution_explore_ndcg", result)
        return metric_dict


class ExploreNDCG(ContributionExploreNDCG):
    def metric_info(self, pos_index, pos_len):
        result = super().metric_info(pos_index, pos_len)
        result = result[pos_len > 0]
        return result
...
class TailPercentage(AbstractMetric):
    ...
    def used_info(self, dataobject):
        ...
        count_items = dataobject.get("data.count_items").numpy()
        return item_matrix, count_items

    def get_tail(self, item_matrix, count_items):
        """Get long-tail percentage through the top-k recommendation list."""
        if self.tail > 1:
            # Absolute threshold: if tail=5, tail items have <= 5 interactions
            threshold = self.tail
        else:
            threshold = np.percentile(count_items, self.tail * 100)
        tail_items = np.where(count_items <= threshold)[0]

        value = np.isin(item_matrix, tail_items).astype(np.float32)
...
class PopularityOverlap(TopkMetric):
    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.items", "data.count_items"]
    ...
    def used_info(self, dataobject):
        recommended_items = dataobject.get("rec.items").numpy()
        item_interaction_counts = dataobject.get("data.count_items").numpy()
        return recommended_items, item_interaction_counts

    def calculate_metric(self, dataobject):
        recommended_items, item_interaction_counts = self.used_info(dataobject)

        items_sorted_by_popularity = np.argsort(-item_interaction_counts, kind="stable")
        global_popularity_ranks = np.empty_like(items_sorted_by_popularity)
        global_popularity_ranks[items_sorted_by_popularity] = np.arange(len(item_interaction_counts))

        recommended_items_popularity_ranks = global_popularity_ranks[recommended_items]

        num_users, max_recommendations = recommended_items_popularity_ranks.shape
        k_cutoffs = np.arange(1, max_recommendations + 1)

        is_popular_enough_for_k = recommended_items_popularity_ranks[:, :, None] < k_cutoffs
        is_within_k_recommendations = np.arange(max_recommendations)[:, None] < k_cutoffs

        popular_items_in_top_k = (is_popular_enough_for_k & is_within_k_recommendations).sum(axis=1)
        overlap_ratios = popular_items_in_top_k.astype(np.float32) / k_cutoffs

        return self.topk_result("popularityoverlap", overlap_ratios)

    def metric_info(self, pos_index):
        pass


class BinnedPopularityMetric(TopkMetric):
    """
    Base class to handle item popularity binning.
    """

    def __init__(self, config):
        super().__init__(config)
        self.bin_edges = [0, 30, 300, 3000, 30000]

    def get_binned_indices(self, item_matrix, item_count):
        items_sorted_by_pop = np.argsort(-item_count, kind="stable")
        global_ranks = np.empty_like(items_sorted_by_pop)
        global_ranks[items_sorted_by_pop] = np.arange(len(item_count))

        recommended_ranks = global_ranks[item_matrix]

        masks = {}
        for i in range(len(self.bin_edges) - 1):
            low, high = self.bin_edges[i], self.bin_edges[i + 1]
            masks[f"{low}_{high}"] = (recommended_ranks >= low) & (recommended_ranks < high)

        last_edge = self.bin_edges[-1]
        masks[f"{last_edge}_plus"] = (recommended_ranks >= last_edge)

        return masks


class BinnedRecall(BinnedPopularityMetric):
    metric_type = EvaluatorType.RANKING
    ...
    def used_info(self, dataobject):
        ...
        topk_idx, pos_len_list = torch.split(rec_mat, [max(self.topk), 1], dim=1)
        # Squeeze ensures pos_len is (891,) and not (891, 1)
        return topk_idx.to(torch.bool).numpy(), pos_len_list.squeeze(-1).numpy()

    def calculate_metric(self, dataobject):
        # Now shapes are guaranteed to match (891 users)
        pos_index, pos_len = self.used_info(dataobject)
        item_matrix = dataobject.get("rec.items").numpy()
        item_count = dataobject.get("data.count_items").numpy()

        bin_masks = self.get_binned_indices(item_matrix, item_count)

        metric_dict = {}
        for bin_name, bin_mask in bin_masks.items():
            binned_hits = pos_index & bin_mask
            result = self.metric_info(binned_hits, pos_len)

            bin_metric_name = f"recall_pop_{bin_name}"
            # self.topk_result handles the averaging automatically!
            metric_dict.update(self.topk_result(bin_metric_name, result))

        return metric_dict

    def metric_info(self, pos_index, pos_len):
        unsqueezed = np.expand_dims(pos_len, 1)
        return np.divide(np.cumsum(pos_index, axis=1), unsqueezed, where=unsqueezed != 0)


class BinnedExploreRecall(BinnedRecall):
    metric_need = ["rec.topk_explore", "rec.items", "data.count_items"]
    ...
    def calculate_metric(self, dataobject):
        ...
        for bin_name, bin_mask in bin_masks.items():
            ...
            bin_metric_name = f"explore_recall_pop_{bin_name}"
            metric_dict.update(self.topk_result(bin_metric_name, result))

        return metric_dict


class BinnedRepeatRecall(BinnedRecall):
    metric_need = ["rec.topk_repeat", "rec.items", "data.count_items"]
    ...
    def calculate_metric(self, dataobject):
        ...
        for bin_name, bin_mask in bin_masks.items():
            ...
            bin_metric_name = f"repeat_recall_pop_{bin_name}"
            metric_dict.update(self.topk_result(bin_metric_name, result))
