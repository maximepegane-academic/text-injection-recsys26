...
class Collector(object):
    ...
    def data_collect(self, train_data):
        ...
        Args:
            train_data (AbstractDataLoader): th e training dataloader which contains the training data.
    ...
    def eval_batch_collect(
        ...
        if self.register.need("rec.topk"):
            ...
            if self.config["optimize"]:
                nnz_mask = positive_i != 0
                expanded_u = positive_u.repeat_interleave(nnz_mask.sum(dim=1))
                nnz_i = positive_i[nnz_mask]
                pos_matrix[expanded_u, nnz_i] = 1
            else:
                pos_matrix[positive_u, positive_i] = 1
            result = torch.cat(
                (
                    torch.gather(pos_matrix, dim=1, index=topk_idx),
                    pos_matrix.sum(dim=1, keepdim=True),
                ),
                dim=1,
            )
        ...
        if self.register.need("rec.topk_repeat"):
            ...
            history = interaction[
                self.config["ITEM_ID_FIELD"] + self.config["LIST_SUFFIX"]
            ]  # n_users x k
            pos_matrix = torch.zeros_like(scores_tensor, dtype=torch.int)
            repeat_mask = batched_isin(positive_i, history)

            if self.config["optimize"]:
                nnz_mask = positive_i != 0
                nnz_repeat_mask = repeat_mask * nnz_mask
                expanded_u = positive_u.repeat_interleave(nnz_repeat_mask.sum(dim=1))
                nnz_i = positive_i[nnz_repeat_mask]
                pos_matrix[expanded_u, nnz_i] = 1
            else:
        ...
        if self.register.need("rec.topk_explore"):
            ...
            pos_matrix = torch.zeros_like(scores_tensor, dtype=torch.int)
            explore_mask = ~batched_isin(positive_i, history)

            if self.config["optimize"]:
                nnz_mask = positive_i != 0
                nnz_explore_mask = explore_mask * nnz_mask
                expanded_u = positive_u.repeat_interleave(nnz_explore_mask.sum(dim=1))
                nnz_i = positive_i[nnz_explore_mask]
                pos_matrix[expanded_u, nnz_i] = 1
            else:
        ...
        if self.register.need("rec.pos_len"):
            pos_len_list = torch.bincount(
                positive_u,
                minlength=scores_tensor.size(0)
            ).to(torch.float32)

            self.data_struct.update_tensor("rec.pos_len", pos_len_list)
        ...
        if self.register.need("data.history"):
            self.history_field = self.config["ITEM_ID_FIELD"] + self.config["LIST_SUFFIX"]
            self.data_struct.update_tensor(
                "data.history", interaction[self.history_field].to(self.device)
            )
