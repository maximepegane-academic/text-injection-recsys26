...
class Dataset(torch.utils.data.Dataset):
    ...
    def _load_inter_feat(self, token, dataset_path):
        ...
        if self.benchmark_filename_list is None:
            ...
            if self.config["optimize"]:
                inter_feat = pl.concat(sub_inter_feats)
            else:
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        if feat is not None:
            if self.config["optimize"]:
                feat = feat.unique(subset=[field], keep="first")
            else:
    ...
    def _load_feat(self, filepath, source):
        ...
        for field_type in head.split(field_separator):
            ...
            if self.config["optimize"]:
                dtype[field_type] = (
                    pl.Float64 if ftype == FeatureType.FLOAT else pl.String
                )
            else:
        ...
        for field in columns:
            ...
            if not ftype.value.endswith("seq"):
                continue
            if self.config["optimize"]:
                df = df.with_columns(pl.col(field).fill_null(value=""))
                if ftype == FeatureType.TOKEN_SEQ:
                    df = df.with_columns(
                        pl.col(field).str.split(seq_separator).list.drop_nulls()
                    )
                elif ftype == FeatureType.FLOAT_SEQ:
                    df = df.with_columns(
                        pl.col(field)
                        .str.split(seq_separator)
                        .list.drop_nulls()
            ...
            if self.config["optimize"]:
                max_seq_len = df.select(pl.col(field).list.len().max()).item()
            else:
    ...
    def _user_item_feat_preparation(self):
        ...
        if self.user_feat is not None:
            if self.config["optimize"]:
                new_user_df = pl.DataFrame({self.uid_field: np.arange(self.user_num, dtype=np.int64)})
                self.user_feat = new_user_df.join(self.user_feat, on=self.uid_field, how="left")
            else:
        ...
        if self.item_feat is not None:
            if self.config["optimize"]:
                new_item_df = pl.DataFrame({self.iid_field: np.arange(self.item_num, dtype=np.int64)})
                self.item_feat = new_item_df.join(self.item_feat, on=self.iid_field, how="left")
            else:
    ...
    def _fill_nan(self):
        ...
        for feat_name in self.feat_name_list:
            ...
            if self.config["optimize"]:
                for field in feat.columns:
                    ftype = self.field2type[field]
                    if ftype == FeatureType.TOKEN:
                        feat = feat.with_columns(
                            pl.col(field).cast(pl.Int64).fill_null(value=0)
                        )
                    elif ftype == FeatureType.FLOAT:
                        feat = feat.with_columns(
                            pl.col(field).cast(pl.Float64).fill_null(strategy="mean")
                        )

                    else:
                        dtype = pl.List(pl.Int64) if ftype == FeatureType.TOKEN_SEQ else pl.List(pl.Float64)
                        feat = feat.with_columns(pl.col(field).fill_null(pl.Series([], dtype=dtype)))
    ...
    def _filter_nan_user_or_item(self):
        ...
        for field, name in zip([self.uid_field, self.iid_field], ["user", "item"]):
            ...
            if feat is not None:
                if self.config["optimize"]:
                    feat = feat.filter(pl.col(field).is_not_null())
                    setattr(self, name + "_feat", feat)
                else:
            ...
            if field is not None:
                if self.config["optimize"]:
                    self.inter_feat = self.inter_feat.with_columns(
                        pl.col(field).drop_nans().drop_nulls()
                    )
                else:
                    dropped_inter = self.inter_feat.index[
                        self.inter_feat[field].isnull()
                    ]
    ...
    def _filter_by_inter_num(self):
        ...
        if self.config["optimize"]:
            ...
            user_bounds = (
                user_inter_num_interval[0][0],
                user_inter_num_interval[0][-1],
            )
            user_strictness = closed_map.get(user_bounds, "both")
            user_lo_bound, user_hi_bound = (
                user_inter_num_interval[0][1],
                user_inter_num_interval[0][2],
            )

            item_bounds = (
                item_inter_num_interval[0][0],
                item_inter_num_interval[0][-1],
            )
            item_strictness = closed_map.get(item_bounds, "both")
            item_lo_bound, item_hi_bound = (
                item_inter_num_interval[0][1],
                item_inter_num_interval[0][2],
            )
            ...
            self.inter_feat = self.inter_feat.filter(
                (
                    ...
                    .over(self.uid_field)
                    .is_between(
                        lower_bound=user_lo_bound,
                        upper_bound=user_hi_bound,
                        closed=user_strictness,
                    )
                )
                & (
                    pl.len()
                    .over(self.iid_field)
                    .is_between(
                        lower_bound=item_lo_bound,
                        upper_bound=item_hi_bound,
                        closed=item_strictness,
                    )
                )
            )
        else:
            ...
            while True:
                ...
                if self.user_feat is not None:
                    ...
                    self.user_feat.drop(
                        self.user_feat.index[dropped_user], inplace=True
                    )

                if self.item_feat is not None:
                    dropped_item = self.item_feat[self.iid_field].isin(ban_items)
                    self.item_feat.drop(
                        self.item_feat.index[dropped_item], inplace=True
                    )
    ...
    def _sample_uid_randomly(self):
        """Take randomly N customer among the transaction data to use; helpful when data has a lot of sample
        This happens AFTER the item and user interactions filtering
        Note : If number is 0 or negative, the original of customer will be user
        """
        unique_uids = self.inter_feat[self.uid_field].unique()
        if self.config["number_random_uid_sample"] > 0:
            self.config["number_random_uid_sample"] = min(
                self.config["number_random_uid_sample"], len(unique_uids)
            )

            rng = np.random.default_rng(seed=self.config["seed"])
            unique_uids = np.sort(unique_uids)
            unique_uids = rng.choice(
                unique_uids, size=self.config["number_random_uid_sample"], replace=False
            )

            if self.config["optimize"]:
                self.inter_feat = self.inter_feat.filter(
                    pl.col(self.uid_field).is_in(unique_uids)
                )
            else:
                mask = self.inter_feat[self.uid_field].isin(unique_uids)
                self.logger.debug(f"[{mask.sum()}] uid kept.")
                self.inter_feat = self.inter_feat[mask]
        if self.config["optimize"]:
            if self.item_feat is not None:
                unique_iid = self.inter_feat[self.iid_field].unique()
                self.item_feat = self.item_feat.filter(
                    pl.col(self.iid_field).is_in(unique_iid)
                )
            if self.user_feat is not None:
                self.user_feat = self.user_feat.filter(
                    pl.col(self.uid_field).is_in(unique_uids)
                )
    ...
    def _reset_index(self):
        ...
        for feat_name in self.feat_name_list:
            feat = getattr(self, feat_name)
            empty_feat = feat.is_empty() if self.config["optimize"] else feat.empty
            if empty_feat:
                raise ValueError(
    ...
    def _filter_inter_by_user_or_item(self):
        ...
        if self.config["optimize"]:
            if self.user_feat is not None:
                self.inter_feat = self.inter_feat.filter(
                    pl.col(self.uid_field).is_in(
                        self.user_feat[self.uid_field].to_list()
                    )
                )
            if self.item_feat is not None:
                self.inter_feat = self.inter_feat.filter(
                    pl.col(self.iid_field).is_in(
                        self.item_feat[self.iid_field].to_list()
                    )
                )
    ...
    @staticmethod
    def pl_factorize(tokens: pl.Series) -> tuple[np.ndarray, np.ndarray]:

        vocab = tokens.unique(maintain_order=True).to_numpy()

        ids = np.arange(1, len(vocab) + 1, dtype=np.int64)

        return vocab, ids

    def _remap_optimized(self, remap_list):
        series_list = []
        for feat, field, ftype in remap_list:
            if ftype == FeatureType.TOKEN:
                series_list.append(feat[field])
            elif ftype == FeatureType.TOKEN_SEQ:
                series_list.append(feat[field].list.explode().drop_nulls())

        global_series = pl.concat(series_list).drop_nulls()
        old_ids, new_ids = self.pl_factorize(global_series)

        old_ids_with_pad = np.insert(old_ids, 0, 0)
        token_id_dict = {t: i for i, t in enumerate(old_ids_with_pad)}

        for feat, field, ftype in remap_list:
            if ftype == FeatureType.TOKEN:
                new_col = (
                    feat[field]
                    .replace_strict(old=old_ids, new=new_ids, default=0)
                    .cast(pl.Int64)
                )
            elif ftype == FeatureType.TOKEN_SEQ:
                new_col = feat[field].list.eval(
                    pl.element()
                    .replace_strict(old=old_ids, new=new_ids, default=0)
                    .cast(pl.Int64)
                )

            field_index = feat.get_column_index(field)
            feat.replace_column(field_index, new_col)

            if field not in self.field2id_token:
                self.field2id_token[field] = old_ids_with_pad
                self.field2token_id[field] = token_id_dict

    # def _remap_optimized(self, remap_list):
    #     for i, (feat, field, ftype) in enumerate(remap_list):
    #         old_ids, new_ids = self.pl_factorize(feat[field])
    #
    #         new_col = feat[field].replace_strict(old=old_ids, new=new_ids)
    #         field_index = feat.get_column_index(field)
    #         feat.replace_column(field_index, new_col)
    #
    #         old_ids = np.insert(
    ...
    #             "[PAD]",
    #         )
    #         new_ids = np.insert(new_ids, 0, 0)
    #         self.field2id_token[field] = old_ids
    #         self.field2token_id[field] = new_ids

    def _remap(self, remap_list):
        """Remap tokens using :meth:`pandas.factorize`, or polars if the optimize flag is set
        ...
        if self.config["optimize"]:
            self._remap_optimized(remap_list)
        else:
    ...
    def counter(self, field):
        ...
        if self.field2type[field] == FeatureType.TOKEN:
            ...
            elif isinstance(self.inter_feat, pl.DataFrame) or isinstance(
                self.inter_feat, pl.LazyFrame
            ):
                return self.inter_feat.lazy().select(pl.col(field).n_unique()).collect()
            else:
    ...
    def item_counter(self):
        ...
        if isinstance(self.inter_feat, pl.LazyFrame) or isinstance(self.inter_feat, pl.DataFrame):
    ...
    def inter_num(self):
        ...
        """
        if isinstance(self.inter_feat, pl.DataFrame) or isinstance(
            self.inter_feat, pl.LazyFrame
        ):
            return self.inter_feat.lazy().select(pl.len()).collect().item()
    ...
    def avg_actions_of_users(self):
        ...
        if isinstance(self.inter_feat, pl.DataFrame) or isinstance(
            ...
            return (
                self.inter_feat.lazy()
                .group_by(self.uid_field, maintain_order=True)
                .agg(pl.col(self.iid_field).len())
                .select(pl.col(self.iid_field).mean())
                .collect()
                .to_numpy()
                .item()
            )
    ...
    def avg_actions_of_items(self):
        ...
        if isinstance(self.inter_feat, pl.DataFrame) or isinstance(
            ...
            return (
                self.inter_feat.lazy()
                .group_by(self.iid_field, maintain_order=True)
                .agg(pl.col(self.uid_field).len())
                .select(pl.col(self.uid_field).mean())
                .collect()
                .to_numpy()
                .item()
            )
    ...
    def item_popularity(self):
        """Get the number of time each item appear in the dataset

        Returns:
            torch.Tensor: Number of time each item appear in the dataset
        """
        if self.config["optimize"]:
            df = self.inter_feat.lazy()
            baskets = (
                df.select(pl.col(self.iid_field).list.explode())
                .filter(pl.col(self.iid_field) != 0)
                .collect()
                .to_torch()
                .flatten()
            )
        else:
            baskets = self.inter_feat[self.iid_field]
            baskets = baskets[baskets > 0]
        src = torch.ones_like(baskets, dtype=torch.float)
        pop = torch.zeros(self.item_num)
        pop.scatter_add_(dim=0, src=src, index=baskets)
        return pop
    ...
    def __getitem__(self, index, join=True):
        if isinstance(self.inter_feat, pl.LazyFrame):
            row_df = self.inter_feat.slice(index, 1).collect()
            batch_dict = {col: row_df[col].to_torch() for col in row_df.columns}
            df = Interaction(batch_dict)
    ...
    def fold_split(self, ratios, group_by=None):
        """Split dataset according to ratio user-wise. i.e. a 80/10/10 split wil result in 80% of users in train, 10% of
        users in validation and 10% of users in test.

        Args:
            ratios (list): List of split ratios. No need to be normalized, Is expected to be a list of 3 int or float.
            group_by (str, optional): Field name that interaction records should grouped by before splitting.
                Defaults to ``None``
        ...
        Note:
            Other than the first one, each part is rounded down.
        """
        self.logger.debug(f"split by fold [{ratios}], group_by= [{group_by}]")
        tot_ratio = sum(ratios)
        ratios = [_ / tot_ratio for _ in ratios]

        if self.config["batch_data_processing"]:
            lf = self.inter_feat.lazy()
            users = lf.select(
                pl.col(group_by).unique().shuffle(seed=self.config["seed"])
            ).collect()

            num_user = users.height
            train_idx = int(num_user * ratios[0])
            val_idx = int(num_user * ratios[1]) + train_idx
            user_with_phase = users.with_columns(
                pl.when(pl.row_index() <= train_idx)
                .then(pl.lit("train"))
                .when((pl.row_index() > train_idx) & (pl.row_index() <= val_idx))
                .then(pl.lit("validation"))
                .otherwise(pl.lit("test"))
                .alias("phase")
            )
            user_partition = user_with_phase.partition_by(
                "phase", include_key=False, as_dict=True
            )

            train_users = user_partition[("train",)][group_by].to_numpy()
            val_users = user_partition[("validation",)][group_by].to_numpy()
            test_users = user_partition[("test",)][group_by].to_numpy()

            train_lf = lf.filter(pl.col(group_by).is_in(train_users))
            val_lf = lf.filter(pl.col(group_by).is_in(val_users))
            test_lf = lf.filter(pl.col(group_by).is_in(test_users))
            next_lf = [train_lf, val_lf, test_lf]
            if not self.config["batch_data_processing"]:
                next_lf = [lf.collect() for lf in next_lf]

            next_ds = [self.copy(_) for _ in next_lf]
            return next_ds
        grouped_inter_feat_index = self._grouped_index(
            self.inter_feat[group_by].cpu().numpy()
        )
        num_user = len(grouped_inter_feat_index)
        user_set = np.array(list(grouped_inter_feat_index), dtype="object")
        shuffled_users = np.random.choice(num_user, size=num_user, replace=False)

        train_split_size = int(num_user * ratios[0])
        train_split_index = shuffled_users[:train_split_size]

        test_split_size = train_split_size + int(num_user * ratios[1])
        test_split_index = shuffled_users[train_split_size:test_split_size]
        val_split_index = shuffled_users[test_split_size:]

        next_index = [[] for _ in range(len(ratios))]

        next_index[0] = user_set[train_split_index]
        next_index[0] = [item for user in next_index[0] for item in user]

        next_index[1] = user_set[test_split_index]
        next_index[1] = [item for user in next_index[1] for item in user]

        next_index[2] = user_set[val_split_index]
        next_index[2] = [item for user in next_index[2] for item in user]
    ...
    def build(self):
        ...
        if self.benchmark_filename_list is not None:
            if self.config["optimize"]:
                datasets = []
                for i in range(len(self.benchmark_filename_list)):
                    part_df = self.inter_feat.filter(
                        pl.col("_benchmark_part") == i
                    )
                    datasets.append(part_df)
                datasets = [self.copy(_) for _ in datasets]
                self._drop_unused_col()
                return datasets
            else:
        ...
        if ordering_args == "RO":
            ...
            pass  # self.sort(by=self.time_field)
        else:
            raise NotImplementedError(
        ...
        if split_mode == "RS":
            ...
            datasets = self.fold_split(
                group_by=self.uid_field,
                ratios=split_args["fold"],
            )
        elif split_mode == "TS":
    ...
    def _dataframe_to_interaction(self, data):
        ...
        """
        new_data = {}
        cols = data.columns if self.config["optimize"] else data
        for k in cols:
            value = data[k].values
