...
from recbole.data import IterEvalDataloader
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        model_dict = {
            ...
            "non_embedding_": sum(
                p.numel() if isinstance(module, torch.nn.Embedding) else 0
                for module in model.modules()
                for p in module.parameters()
                if p.requires_grad
            ),
        }
        self.wandblogger.log_eval_metrics(model_dict, head="eval")
        ...
        self.saved_prediction_path = os.path.join(
            self.checkpoint_dir,
            f"{config["run_name"]}_predictions.csv",
        )
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self.wandblogger.log_metrics({"loss": loss}, head="full_train_loss")
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                if hasattr(self.model, "post_training_step"):
                    self.model.post_training_step()
    ...
    def evaluate(
        ...
        if isinstance(eval_data, FullSortEvalDataLoader) or isinstance(
            eval_data, IterEvalDataloader
        ):
        ...
        k = max(self.config["topk"])
        num_sample = 0
        pred_schema = {
            "shopper_id": pl.Int64,
            "product_id": pl.List(pl.Int64) if self.config["optimize"] else pl.Int64,
            "transaction_date": pl.Int64,
        }
        pred_schema.update({f"top{i}_pred": pl.Int64 for i in range(1, k + 1)})
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            temp = pl.DataFrame(
                (
                    interaction[self.model.USER_ID].detach().cpu().numpy(),
                    positive_i.detach().cpu().numpy(),
                    (
                        interaction[self.model.TIME_FIELD].detach().cpu().numpy()
                        if hasattr(self.model, "TIME_FIELD")
                        else None
                    ),
                    *[
                        scores.topk(max(self.config["topk"]))
                        .indices[:, i]
                        .detach()
                        .cpu()
                        .numpy()
        ...
        if self.config["optimize"]:
            pred_df = pred_df.with_columns(
                pl.col("product_id").list.filter(pl.element() != 0)
            ).explode("product_id")
        # pred_df.write_csv(self.saved_prediction_path)
...
class TIFUKNNTrainer(TraditionalTrainer):
    r"""TIFUKNNTrainer is designed for the TIFUKNN model, and adds neigbor precomputing and index building"""

    def __init__(self, config, model):
        super(TIFUKNNTrainer, self).__init__(config, model)
        self.eval_step = 1  # Set the eval step to 1 since we only have 1 epoch
    ...
    def fit(
        ...
        r"""Train the model based on the train data and the valid data.
            Essentially the same method as fit from TraditionalTrainer but calls the "build_ANN_index" model's method
            at the end of training. The chosen ANN algorithm needs to build special indexing once all the neighbors have
            been gathered to allow for fast inference afterwards
        Args:
            train_data (DataLoader): the train data
            valid_data (DataLoader, optional): the valid data, default: None.
                                               If it's None, the early_stopping is invalid.
        ...
        self.model.check_unseen_user()
        self.model.build_ANN_index()
        self.model.neighbors_precomputing()
