...
def get_flops(model, dataset, device, logger, transform, verbose=False):
    ...
    if model.type == ModelType.DECISIONTREE:
        return 1
    if model.__class__.__name__ in ["Pop", "TIFUKNN"]:
        return 1
    ...
    inter = dataset.inter_feat[1].to(device)
...
def batched_isin(tensor_1, target_tensor):
    if tensor_1.dim() == 1:
        matches = (tensor_1.unsqueeze(1) == target_tensor).any(dim=1)
    else:
        matches = (tensor_1.unsqueeze(2) == target_tensor.unsqueeze(1)).any(dim=2)
    return matches & (tensor_1 != 0)


def build_text_col(config, dataset):
    if dataset.item_feat is not None:
        if isinstance(config["TEXT_FIELD"], list):
            name = dataset.iid_field + "_name"
            fields = config["TEXT_FIELD"]
            dataset.item_feat = dataset.item_feat.with_columns(
                pl.concat_str(
                    [
                        pl.col(name).replace(
                            {i: ("" if i == 0 else val) for i, val in enumerate(dataset.field2id_token[name])},
                            default=""
                        ).cast(pl.Utf8)
                        for name in fields
                    ],
            ...
            dataset.text_field = name
        else:
            dataset.text_field = config["TEXT_FIELD"]
        if config["EMPTY_TEXT"]:
            dataset.item_feat = dataset.item_feat.with_columns(
                pl.col(dataset.text_field).str.replace(config["EMPTY_TEXT"], "")
            )
    else:
        dataset.text_field = None
